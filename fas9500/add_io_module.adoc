...
---
= Add an I/O module to your FAS9500 system to enhance network connectivity
:icons: font
:imagesdir: ../media/

[.lead]
You can add an I/O module to your FAS9500 system by either replacing a NIC or storage adapter with a new one in a fully-populated system, or by adding a new NIC or storage adapter into an empty chassis slot.
...
+
NOTE: Make sure that you install blanks in any unused I/O slots to prevent possible thermal issues.
...
. If you are using slot 3 or 7 for networking, use the `storage port modify -node __<node name>__ -port __<port name>__ -mode network` command to convert the slot for networking use.
...
a|
The LIFs automatically migrate when the controller module is shut down.
a|
