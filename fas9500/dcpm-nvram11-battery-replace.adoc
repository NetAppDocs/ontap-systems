...
---
= Replace the DCPM module and NVRAM11 battery in your FAS9500 system
:icons: font
:imagesdir: ../media/
...
You must have a replacement DCPM module in-hand before removing the failed module from the chassis. You must replace it within five minutes of removal. After the DCPM module is removed from the chassis, there is no shutdown protection for the controller module that owns the DCPM module, other than failover to the other controller module.
...
+
The LED is steady amber if the module is faulty.
+
...
+
image::../media/drw_a900_remove_NV_battery.svg[Replace the NV (DCPM) battery]
+
