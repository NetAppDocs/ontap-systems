...
summary: "Replace the controller in your FAS2600 system by removing the impaired controller, moving FRU components to the replacement controller module, installing the replacement controller module in the chassis, and then booting the system to Maintenance mode."
---
= Replace the controller in a FAS2600 system
:icons: font
:imagesdir: ../media/

[.lead]
Replace the controller in your FAS2600 system by removing the impaired controller, moving FRU components to the replacement controller module, installing the replacement controller module in the chassis, and then booting the system to Maintenance mode.

link:https://youtu.be/2_uWRt_iJqk[AFF FAS2600 controller replacement video]
...
+
image::../media/drw_2600_boot_media_repl_animated_gif.png[Replacing the boot media]
...
+
The DIMM fits tightly in the slot, but should go in easily. If it doesn't, realign the DIMM with the slot and reinsert it.
+
...
To move the caching module, referred to as the M.2 PCIe card on the controller label, from the old controller module to the replacement controller module, follow this sequence of steps.

You must have the new controller module ready so that you can move the caching module directly from the old controller module to the corresponding slot in the new one. All other components in the storage system must be functioning properly. If they aren't, contact technical support.
...
+
image::../media/drw_2600_fcache.png[Removing a caching module]
...
For HA pairs with two controller modules in the same chassis, the order in which you install the controller modules is especially important because a controller module attempts to reboot as soon as it is fully seated in the chassis.
...
+
NOTE: You connect the rest of the cables to the controller module later in this procedure.
