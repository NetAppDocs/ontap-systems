...
summary: "Hot-swap a power supply in your FAS2600 system by turning off, disconnecting, and removing the failed unit, then installing and reconnecting the replacement."
---
= Hot-swap a power supply unit in a FAS2600 system
:icons: font
:imagesdir: ../media/

[.lead]
Hot-swap a power supply in your FAS2600 system by turning off, disconnecting, and removing the old power supply, then installing, connecting, and turning on the replacement power supply.
...
+
NOTE: Cooling is integrated with the power supply, so you must replace the power supply within two minutes of removal to prevent overheating due to reduced airflow. Because the chassis provides a shared cooling configuration for the two HA nodes, a delay longer than two minutes shuts down all controller modules in the chassis. If both controller modules do shut down, make sure that both power supplies are inserted, turn both off for 30 seconds, and then turn both on.

* The number of power supplies in the system depends on the model.
* Power supplies are auto-ranging.

link:https://youtu.be/1xWZfXpXMNE[AFF FAS2600 power supply replacement video^]
