...
summary: "When replacing a drive drawer in a shelf, you can either halt the entire storage system (HA pair) or keep the HA pair up and running."
---
= Replace a DS460C drive drawer
:icons: font
:imagesdir: ../media/
...
* This procedure applies to shelves having DCM drive drawers and/or DCM2 or DCM3 drive drawers. (Shelves will also have two IOM12 modules, two IOM12B modules, or two IOM12C modules.)
+
When a DCM, DCM2, or DCM3 drive drawer fails, you receive a DCM, DCM2, or DCM3 drive drawer to replace it.

[NOTE]
====
When you replace a failed drive drawer with a newer DCM2 or DCM3 drive drawer, ensure that ONTAP software and the IOM12, IOM12B, or IOM12C module firmware are running the minimum versions required to support DCM2 or DCM3 drawers.

The IOM12, IOM12B, or IOM12C FW upgrade can be done before or after replacing a drive drawer. This procedure has you upgrade the FW as part of the preparation for the drawer replacement procedure.
====
...
. If your shelf will have a combination of IOM12, IOM12B, or IOM12C modules with any number of DCM3 drive drawers, your system must be running the following minimum versions of ONTAP:
...
.. Move all of the data from the data aggregates located on all disk drives on the shelf.
+
