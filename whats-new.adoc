...
I/O module hot-swap procedures are now available for multiple platform families, allowing you to replace I/O modules without shutting down the system.

*Supported with ONTAP 9.17.1 or later:*

Hot-swap is supported for two-node HA pairs with the following requirements:

* The cluster and HA I/O module must be in slot 4
* The impaired controller must be in a takeover state
* Only two-node (switchless or switched) cluster configurations are supported

The following platforms support hot-swap with ONTAP 9.17.1 or later:

* AFF A-Series: A20, A30, and A50
* ASA r2: A20, A30, and A50  
* AFF C-Series: C80

*Supported with ONTAP 9.18.1 GA or later:*

Hot-swap is supported with greater flexibility:

* Ethernet I/O modules can be hot-swapped in any slot
* Supported for clusters of any size (switchless or switched)
* Controllers can be up and running I/O or in a takeover state
* All nodes in the cluster must be running the same ONTAP version or different patch levels of the same version

The following platforms support hot-swap with ONTAP 9.18.1 GA or later:

* AFF A-Series: A1K, A20, A30, A50, A70, and A90
* ASA r2: A1K, A20, A30, A50, A70, and A90  
* AFF C-Series: C80
* FAS: FAS70 and FAS90

NOTE: I/O modules with ports used for storage or MetroCluster are not hot-swappable on any platform or ONTAP version.
...
link:endofavail/index.html[View end-of-availability hardware].

// 2026-01-28 ontap-systems-internal/issues/1389
