...
[.lead]
After connecting the network cables, you are ready to apply power to the shelf and to the controllers.

== Step 1: Power on the shelf and assign shelf ID
In ONTAP storage systems, each NS224 shelf is distinguished by a unique shelf ID. This ID ensures that the shelf is distinct within your storage setup, whether it’s a standalone platform or part of a cluster. By default, shelf IDs are assigned as ‘00’ and ‘01’, but you may need to adjust these IDs to maintain uniqueness across your network.
...
+
image::../media/drw_a1k_cable_console_switch_ieops-1702.svg[]
