...
[.lead]
In an AFF A1K storage system, the manual recovery of the boot image involves using a USB drive to reinstall ONTAP onto the replacement boot media. You must download the appropriate ONTAP recovery image from the NetApp Support Site and copy it to a USB drive. This prepared USB drive is then used to perform the recovery and restore the system to operational status.
