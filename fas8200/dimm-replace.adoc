...
summary: "You must replace a DIMM in the controller module when your storage system reports excessive correctable error correction code (CECC) errors identified by Health Monitor alerts, or uncorrectable ECC errors caused by a single DIMM failure that prevents the storage system from booting ONTAP."

---
= Replace a DIMM in your FAS8200 system to fix memory errors
:icons: font
:imagesdir: ../media/

[.lead]
You must replace a DIMM in the controller module when your storage system reports excessive correctable error correction code (CECC) errors identified by Health Monitor alerts, or uncorrectable ECC errors caused by a single DIMM failure that prevents the storage system from booting ONTAP.

All other components in the system must be functioning properly. If they're not, contact technical support.
