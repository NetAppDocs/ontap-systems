...
summary: "Replace the failed boot media on a FAS8200 controller module. The FAS8200 system supports only manual boot media recovery procedures. Automated boot media recovery is not supported."
---
= Replace the boot media in your FAS8200 system
:icons: font
:imagesdir: ../media/

[.lead]
Replace the failed boot media on a FAS8200 controller module. This procedure includes removing the controller module from the chassis, physically replacing the boot media component, transferring the boot image to the replacement media using a USB flash drive, and restoring the system to normal operation.
...
+
NOTE: The tar.gz file must be placed on a FAT32 formatted partition that is a minimum of 4GB. While FAT32 partitions can be as large as 2TB, Windows built-in tools (for example, DiskPart) cannot format FAT32 partitions larger than 32GB. 
+  
...
+
The system implements the changes when it boots.
