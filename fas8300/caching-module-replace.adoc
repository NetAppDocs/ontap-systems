...
summary: "You must replace the caching module in the controller module when your system registers a single AutoSupport message that the module has gone offline."
---
= Replace the caching module in your FAS8300 or FAS8700 system
:icons: font
:imagesdir: ../media/
...
+
NOTE: Run the `system controller flash-cache show` command if you don't know the Flash Cache device ID.
+
...
NOTE: Slot 6 is only available in the FAS8300 VER2 controller.
...
* Although the contents of the caching module are encrypted, it is a best practice to erase the contents of the module before replacing it. For more information, see the https://mysupport.netapp.com/info/web/ECMP1132988.html[Statement of Volatility] for your system on the NetApp Support Site.
+
NOTE: You must log into the NetApp Support Site to display the _Statement of Volatility_ for your system.

* All other components in the storage system must be functioning properly. If they aren't, contact technical support.
...
. Locate the failed caching module using the FRU map on the controller module, and remove it:
+
Depending on your configuration, there might be zero, one, or two caching modules in the controller module.  Use the FRU map inside the controller module to help locate the caching module. 
...
image::../media/drw_A400_Install_controller_source.png[Installing the controller]
