...
summary: "You must replace the NVDIMM in the controller module when your system registers that the flash lifetime is almost at an end or that the identified NVDIMM is not healthy in general."
---
= Replace the NVDIMM in your FAS8300 or FAS8700 system
:icons: font
:imagesdir: ../media/
...
All other components in the system must be functioning properly. If they are not, you must contact technical support.
...
To replace the NVDIMM, you must locate it in the controller module using the FRU map on the top of the slot 1 riser.

* The NVDIMM LED blinks while destaging contents when you halt the system. After the destage is complete, the LED turns off.
* Although the contents of the NVDIMM are encrypted, it is a best practice to erase the contents of the NVDIMM before replacing it. For more information, see the https://mysupport.netapp.com/info/web/ECMP1132988.html[Statement of Volatility] on the NetApp Support Site.
+
...
+
The NVDIMM fits tightly in the slot, but should go in easily. If it doesn't, realign the NVDIMM with the slot and reinsert it.
+
