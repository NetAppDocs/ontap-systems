...
summary: "You must replace the DIMM in the controller when your storage system encounters errors such as excessive correctable error correction codes or uncorrectable errors, typically caused by a single DIMM failure preventing the storage system from booting ONTAP."
---
= Replace the DIMM in a FAS8300 or FAS8700 system to fix memory errors
:icons: font
:imagesdir: ../media/

[.lead]
You must replace the DIMM in the controller when your storage system encounters errors such as excessive CECC (Correctable Error Correction Code) errors based on Health Monitor alerts, or uncorrectable ECC errors. These errors are typically caused by a single DIMM failure that prevents the storage system from booting ONTAP.

All other components in the system must be functioning properly. If a component is not functioning properly, you must contact technical support.
...
The number and location of DIMMs in your system depend on the model of your system. Refer to FRU map on the air duct for more information.
...
.. Firmly push the controller module into the chassis using the locking latches until the locking latches begin to rise.
+
