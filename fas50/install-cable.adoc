...
* You cable each controller to each IOM12 module on the SAS shelf (DS460C shelves are shown) using the storage cables that came with your storage system, which could be the following cable type:
+
*100 GbE QSFP28 copper cables* ????? Check FAS70?
+
...
.. Cable controller A port 3d to IOMB port 3.
+
image:../media/drw_isi_fas50_2_ds460c_controller_a_cabling_ieops-2170.svg[Controller A ports 3a and 3d cabled to one DS460C shelf????]

. Cable controller B to the shelf: ????
.. Cable controller B port 3a to IOMB port 1.
.. Cable controller B port 3d to IOMA port 3.
+
image:../media/drw_isi_fas50_2_ds460c_controller_b_cabling_ieops-2171.svg[Controller B ports 3a and 3d cabled to one DS460C shelf???]
