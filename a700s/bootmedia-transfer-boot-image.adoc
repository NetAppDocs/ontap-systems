...
[.lead]
Transfer the boot image to the replacement boot media on an AFF A700s system using either the secondary boot media or a USB flash drive. The secondary boot media is the primary method. Use a USB flash drive only if the secondary boot media restoration fails or the `image.tgz` file is missing.
...
The image on the secondary boot media must contain an `image.tgz` file and must not be reporting failures. If the `image.tgz` file is missing or the boot media reports failures, you cannot use this procedure. Use Option 2 to transfer the boot image using a USB flash drive.
