...
[.lead]
Transfer the boot image to the replacement boot media on an AFF A700s system using either the secondary boot media or a USB flash drive. This procedure includes restoring from the image on the secondary boot media as the primary method, or using a USB flash drive if the secondary boot media restoration fails or the image.tgz file is missing.
