...
summary: "Boot the ONTAP recovery image from the USB drive on an AFF A700s system to restore the boot media. The AFF A700s system supports only manual boot media recovery procedures. Automated boot media recovery is not supported."
---
= Boot the recovery image on your AFF A700s system
:icons: font
:imagesdir: ../media/

[.lead]
Boot the ONTAP recovery image from the USB drive on an AFF A700s system to restore the boot media. This procedure includes booting from the USB flash drive, restoring the file system, verifying environmental variables, and returning the controller to normal operation after boot media replacement.
