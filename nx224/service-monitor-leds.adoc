...
---
= Monitor NX224 shelf LEDs
:icons: font
:imagesdir: ../media/

[.lead]
You can monitor the health of your shelf by understanding the location and status conditions of the LEDs on your drive shelf components.

* The location (blue) LEDs, on a shelf's operator display panel (ODP) and both NVMe shelf modules (NSMs), can be activated to aid in physically locating the shelf that needs servicing: `storage shelf location-led modify -shelf-name _shelf_name_ -led-status on`
+
...
image::../media/drw_generic_psu_ac_terra_cotta_ieops-2370.svg[Power supply power activity LED, width=400px]
