...
---
= Install your AFF A250 system using detailed step-by-step instructions
:icons: font
:imagesdir: ../media/

[.lead]
Install your AFF A250 storage system using these detailed step-by-step instructions, from preparing the site to bringing up the ONTAP cluster.
...
.Steps
. Use the the 25 GbE cluster interconnect cable to connect the cluster interconnect ports e0c to e0c and e0d to e0d.  
+
...
NOTE: link:https://hwu.netapp.com[NetApp Hardware Universe^] slot priority for host network cards (Fibre Channel or 25 GbE) is slot 2. However, if you have both cards, the Fibre Channel card goes in slot 2 and the 25 GbE card goes in slot 1 (as shown in the options below). If you have an external shelf, the storage card goes in slot 1, the only supported slot for shelves.
...
--
.Option 2: Cable to 25 GbE data or host network
--
25 GbE ports on the controllers are connected to 25 GbE data or host network switches.
...
+
image:../media/drw_affa250_1shelf_cabling_a.png[Controller 1 cabling]

. Cable controller B to the shelf.
+
image:../media/drw_affa250_1shelf_cabling_b.png[Controller 2 cabling]
--
...
NOTE: For information about how to establish continuous connectivity of a controller console port for remote management, see Knowledge Base article, https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/How_to_connect_an_ONTAP_controller_console_port_for_remote_management[How to connect an ONTAP controller console port for remote management^].
...
// Dec 3, 2025: ontap-systems 405 https://github.com/NetAppDocs/ontap-systems/issues/405#issuecomment-3605370099