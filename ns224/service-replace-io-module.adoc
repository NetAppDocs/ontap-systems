...
+
image::../media/drw_g_and_t_handles_remove_ieops-1837.svg[Remove NSM module.]
...
. Remove the failed I/O module from the NSM module:
+
...
.. Pull the I/O module out of the NSM module using the port label tab on the left and the thumbscrew.
...
+
image::../media/drw_g_and_t_handles_reinstall_ieops-1838.svg[Replace NSM module.]
...
. Verify that the attention (amber) LEDs on the NSM module containing the failed I/O module and the shelf operator display panel are no longer illuminated
+
The NSM module attention LEDs turn off after the NSM module reboots and no longer detects an I/O module issue. This can take three to five minutes.
