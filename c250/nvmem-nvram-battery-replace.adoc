---
permalink: c250/nvmem-nvram-battery-replace.html
sidebar: sidebar
keywords: aff c250, nvmem battery replacement
summary: "To replace an NVMEM battery in the system, you must remove the controller module from the system, open it, replace the battery, and close and replace the controller module."
---
= Replace the NVMEM battery in an AFF C250 system
:icons: font
:imagesdir: ../media/

[.lead]
Replace a failed NVMEM battery in an AFF C250 system by removing the controller module, replacing the battery, and reinstalling the controller module.

include::../_include/250_nvmem-nvram-battery-replace.adoc[]
