---
permalink: a150/chassis-replace-shutdown.html
sidebar: sidebar
keywords: aff a150, a150, aff, replace, chassis, shut down, controller, metrocluster, power, supply, ha, state, controller, configuration, switch, aggregates, replace, move, remove, swap
summary: "To replace the chassis, you must shutdown the controllers."
---
= Shut down your AFF A150 controllers to replace the chassis
:icons: font
:imagesdir: ../media/

[.lead]
Shut down the controllers in your AFF A150 storage system to prevent data loss and ensure system stability when replacing the chassis.

== Option 1: Most configurations

include::../_include/shutdown_chassis.adoc[]

== Option 2: Controller is in a MetroCluster configuration

include::../_include/shutdown_any_mcc_supported.adoc[]
