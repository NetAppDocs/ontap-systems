...
summary: To complete the controller replacement for your AFF C30 or AFF C60 storage system, first restore the NetApp Storage Encryption configuration, if necessary, and install the required licenses on the new controller. Next, confirm that the logical interfaces LIFs are reporting to their home ports and perform a cluster health check. Finally, register the new controller's serial number and then return the failed part to NetApp
---
= Complete the controller replacement for your AFF C30 or AFF C60 storage system
:icons: font
:imagesdir: ../media/
