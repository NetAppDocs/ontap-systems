...
summary: The NVMe SSD FlashCache module (FlashCache or caching module) contains one or two FlashCache drive carriers with a single SSD caching drive integrated into the carrier. 
---
= Replace the caching module or FlashCache drive carrier - FAS70 and FAS90
:icons: font
:imagesdir: ../media/

[.lead]
The NVMe SSD FlashCache module (FlashCache or caching module) contains one or two FlashCache drive carriers with a single SSD FlashCache drive integrated into each FlashCache drive carrier. 

You can perform either of the following procedures depending on what needs to be replaced: the entire caching module or just a FlashCache drive carrier.

* Replace the <<Replace the FlashCache module>>
* Replace the <<Replace a FlashCache drive carrier>>

== Replace the FlashCache module
The FlashCache module is located in slot 6 and houses the two FlashCache drive carriers. You cannot hot-swap the FlashCache module. 

.Before you begin

* Ensure your storage system has the appropriate operating system for the replacement caching module.
...
Perform the following steps to replace the caching module.
...
image::../media/drw_fas70-90_caching_module_remove_IEOPS-1772.svg[Remove the caching module]
...
== Replace a FlashCache drive carrier
The FlashCache drive carriers are located in slot 6-1 or in slot 6-2 or in both slot 6-1 and slot 6-2. 

You can hot-swap the individual FlashCache drive carriers with drives of the same capacity from the same vendor or from a different supported vendor.

.Before you begin
* Ensure the replacement FlashCache drive carrier has the same capacity as the failed one, from the same vendor or from a different supported vendor.
* Confirm all other components are functioning properly; if not, you must contact technical support.

.About this task
The drives in the carrier are not field replaceable units (FRU). You must replace the entire FlashCache drive carrier. 
...
image::../media/drw_fas70-90_flashcache_drive_carrier_remove-ieops-1773.svg[Remove the FlashCache drive carrier]
