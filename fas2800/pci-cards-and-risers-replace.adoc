...
[.lead]
Replace the mezzanine card in your FAS2820 system if the card fails or if you want to change the card protocol.
...
* Write down the ifgrp and VLAN information before the shutdown.
* If changing mezzanine card types, change LIF and port settings after the controller restarts.
* Verify all other system components work. If the issue persists, call technical support.
...
After the FAS2820 system restarts, change the mezzanine card settings to match the new card if you replaced it with a different type.
...
. Rebuild any ifgrps and VLANs tied to the old mezz ports.
...
. Move the LIFs back to their home ports and verify the configuration.
. Delete LIFs that are no longer needed.
--

See link:https://docs.netapp.com/us-en/ontap/san-admin/index.html[SAN management overview] for more detailed information about configuring Ethernet in ONTAP.
...
. Enable FCP on relevant SVMs. Create new data and WWPN LIFs.
. Update fabric zoning and host paths. Update port set, igroup, and LUN mappings as needed.
...
. Verify no LIFs are homed on removed Ethernet ports, all FC LIFs are up and serving I/O, and ASUP monitoring reflects the new configuration.
...
. Bring the controller back to normal operation by restoring its storage: `storage failover giveback -ofnode _impaired_node_name_`
