...
summary: "The boot media in your FAS2820 system stores essential firmware and configuration data. To replace it, you remove and open the impaired controller module, locate and replace the boot media, transfer the boot image to a USB drive, and then boot the controller."
---
= Replace the boot media in your FAS2820 system using the manual boot recovery process
:icons: font
:imagesdir: ../media/

[.lead]
The boot media in your FAS2820 system stores essential firmware and configuration data. To replace it, you remove and open the impaired controller module, locate and replace the boot media, transfer the boot image to a USB drive, insert the USB drive in the controller, and then boot the controller.
...
Locate the boot media, which is under the mezzanine card in the controller module, and follow the directions to replace it.
...
.. Use https://kb.netapp.com/on-prem/ontap/DM/Encryption/Encryption-KBs/How_to_determine_if_the_running_ONTAP_version_supports_Data_At_Rest_Encryption[How to determine if your ONTAP version supports NetApp Volume Encryption (NVE)^] to determine if volume encryption is currently supported.
