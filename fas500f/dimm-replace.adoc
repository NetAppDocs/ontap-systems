...
summary: "Replace the DIMM in a FAS500f system when the system reports excessive correctable or uncorrectable ECC memory errors through Health Monitor alerts. A failing DIMM can prevent the storage system from booting ONTAP, so replacing it restores stable operation. This procedure includes shutting down the impaired controller, replacing the DIMM, and returning the failed part to NetApp."
---
= Replace the DIMM in a FAS500f system to fix memory errors
:icons: font
:imagesdir: ../media/

[.lead]
Replace the DIMM in a FAS500f system when the system reports excessive correctable or uncorrectable ECC memory errors through Health Monitor alerts. A failing DIMM can prevent the storage system from booting ONTAP, so replacing it restores stable operation. This procedure includes shutting down the impaired controller, replacing the DIMM, and returning the failed part to NetApp.

If any other components in the system are not functioning properly, contact technical support.
...
. Grasp the controller module with both hands and gently pull it out of the chassis, then set it on a flat, stable surface.
...
+
The DIMMs should fit tightly in the socket. If a DIMM doesn't fit tightly, reinsert it to realign it with the socket.
...
. Insert the controller module into the chassis:
 .. Verify that the latching mechanism arms are locked in the fully extended position.
 .. With both hands, align and gently slide the controller module into the latching mechanism arms until it stops.
