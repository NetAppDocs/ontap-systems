...
Verify the HA configuration settings and disk list before returning the system to service.
...
. If the displayed system state of the controller module does not match your system configuration, set the `HA` state for the controller module: 
+
...
`ha-config show`
+
. Verify that the adapter lists the paths to all disks: `storage show disk -p`
+
