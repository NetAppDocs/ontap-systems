// include used for ASAr2 only since drive ownership is automatically assigned. There is no ability to manually assign drives; therefore, those steps are not in this procedure (like they are for AFF and FAS).
