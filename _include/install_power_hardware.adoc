...
Each NS224 shelf is distinguished by a unique shelf ID. This ID ensures that the shelf is distinct within your storage system setup. By default, shelf IDs are assigned as ‘00’ and ‘01’, but you may need to adjust these IDs to maintain uniqueness across your storage system. If your system has onboard storage, that storage is identified as shelf ID 00.

.About this task
* A valid shelf ID is 00 through 99, or 01 through 99 if the system has onboard storage. 
...
The first number on the digital display stops blinking.
 ...
 .. Press and release the shelf ID button to advance the number until you reach the desired number from 0 to 9, or 1 to 9 if the system has onboard storage.
+
