...
----
+ 
- If the command returns `There are no entries matching your query.` continue to <<check-missing-drives,step 3b>>. 
...
+ 
- If the command returns `There are no entries matching your query.` continue to <<remove-power-cables,step 4>>. 
- If the command returns any other results, collect the AutoSupport data from both controllers and contact NetApp Support for further assistance.
+
[source,cli]
----
