...
// NEW for initial FAS50 release only:
...
. Gently remove the bezel from the front of the storage system.
. Remove the caching modules and any drive blanks:
+
NOTE: Keep track of what drive bay each caching module and drive blank was removed from because they must be installed in the same drive bays in the replacement chassis.
+
...
. Set the caching modules aside on a static-free cart or table.
...
+
NOTE: The caching modules and drive blanks must be installed in the same drive bays in the replacement chassis.
