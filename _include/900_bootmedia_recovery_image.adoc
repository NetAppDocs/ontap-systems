...
. From the LOADER prompt, boot the recovery image from the USB flash drive: `boot_recovery <image_name>`
+
...
.. If the system does not use encyption, select _Option 1 Normal Boot._, otherwise go to link:bootmedia_encryption_restore.html[Restore key managers].
