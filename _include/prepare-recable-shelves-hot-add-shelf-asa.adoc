...
When you hot-add shelves to an ASA A1K HA pair, you need to recable existing shelves to balance connectivity across multiple RoCE-capable I/O modules. 
...
When you hot-add a second shelf to an ASA A70 or ASA A90 HA pair, you need to recable the first shelf across the RoCE-capable I/O modules in slot 11 and slot 8 on each controller. 
...
When you hot-add a second shelf to an ASA A800 or ASA C800 HA pair, you need to recable the first shelf across the two sets of RoCE-capable ports in slot 5 and slot 3 on each controller. 
...
When you hot-add a second shelf to an ASA A400 or ASA C400 HA pair, you need to recable the first shelf across two sets of RoCE-capable ports on each controller. 

* For the ASA A400, the procedure assumes the existing shelf is cabled to RoCE-capable onboard ports e0c/e0d on each controller, and you recable across onboard e0c/e0d and slot 5. 

* For the ASA C400, the procedure assumes the existing shelf is cabled to RoCE-capable ports in slot 4 on each controller, and you recable across slot 4 and slot 5.
...
When you hot-add shelves to an ASA A900 HA pair, you need to recable existing shelves to balance connectivity across multiple RoCE-capable I/O modules. 
...
When you hot-add a second shelf to an ASA A30 or ASA A50 HA pair, you need to recable the first shelf across the RoCE-capable I/O modules in slot 3 and slot 1 on each controller. 
