Before returning the _replacement_ node to service, you should verify that the LIFs are on their home ports, and register the serial number of the _replacement_ node if AutoSupport is enabled, check the health of your cluster, close any maintenance windows, and reset automatic giveback.
...
** If AutoSupport is not enabled, contact https://mysupport.netapp.com[NetApp Support^] to register the serial number.
