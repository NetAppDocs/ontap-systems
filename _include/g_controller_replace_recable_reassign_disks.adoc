...
. If your storage system has Encryption configured, you must restore Storage or Volume Encryption functionality using the following procedure to reboot the storage system:
...
. If your storage system does not have Encryption configured, complete the following procedure to reboot the storage system:
...
. If you disabled automatic takeover on reboot, enable it from the healthy controller: `storage failover modify -node replacement-node-name -onreboot _true_`
