...
. Verify that the external key manager is configured:
+
`security key-manager external show -instance`
+
Review the output to confirm that the KMIP servers are configured correctly.

. Verify that all keys are accessible:
+
`security key-manager key query -restored false`
+
The command should return no results. If any results appear, contact NetApp Support.
+
*On the partner controller:*
