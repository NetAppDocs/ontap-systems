...
. Reboot the controller module from the LOADER prompt: `bye` 
...
a| If I/O module is a... 
a| Then...

a| NIC module
a| Use the `storage port modify -node *_<node name>__ -port *_<port name>__ -mode network` command for each port.

a| Storage module
a| Install and cable your NS224 shelves, as described in link:../ns224/hot-add-shelf-overview.html[Hot-add workflow].
