 ...
 ** If <lno-DARE> or <1Ono-DARE> is displayed in the command output, the system does not support NVE, go to link:../a1k/bootmedia-shutdown.html[Shut down the impaired controller].

 ** If <lno-DARE> is not displayed in the command output, and the system is running ONTAP 9.6 or later, continue to the next section, <<Check NVE or NSE on systems running ONTAP 9.15 and later>>.  *CHECK THIS*  it wille be changing
