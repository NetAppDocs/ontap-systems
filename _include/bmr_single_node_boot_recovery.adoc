...
*NOTE:* If the partner node is unavailable, this UUID information can also be retrieved from the configured key server using the Mroot-AK key.

.. Enter the values for Keystore UUID and Cluster UUID when prompted.
+

.. Verify the attributes for the Cluster UUI and the Keystore UUID.
* For the Cluster UUID: `x-NETAPP-ClusterName: <cluster name>` 
* For the Keystore UUID: `x-NETAPP-KeyUsage: "MROOT-AK"` 
