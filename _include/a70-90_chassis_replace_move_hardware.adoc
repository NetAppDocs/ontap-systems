...
You need to remove all four (two per controller) of the PSUs in the controller before removing the controller.  Removing them lightens the overall weight of each controller.
...
* If the NV LED is flashing, wait for the flashing to stop. If flashing continues for longer than 5 minutes, contact http://mysupport.netapp.com/[NetApp Support Site^] for assistance.
...
+
NOTE: Always use two hands to support the drive weight when removing a drive.
+
NOTE: Drives are fragile. Handle them as little as possible to prevent damage to them.

.. Keep track of what drive bay each drive was in and set the drive aside on a static-free cart or table.
