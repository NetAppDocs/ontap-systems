...
. If you disabled automatic giveback, restore it: `storage failover modify -node * -auto-giveback _true_`

// If you disabled automatic giveback from the console of the healthy controller, restore it: `storage failover modify -node local -auto-giveback _true_`
...
.. From the LOADER prompt, reboot the controller:`bye`
