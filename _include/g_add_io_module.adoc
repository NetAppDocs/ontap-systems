...
You can add an I/O module into an empty slot in a controller.
...
. Remove the target slot blanking module from the controller OR IS it controllers - one at a time???:
...
You can change an I/O module in an I/O slot in a fully-populated storage system by removing an existing I/O module and replacing it with a different I/O module.
...
To replace an I/O module, locate it within the controller and follow the specific sequence of steps.
...
.. Gently slide the module into the slot all the way into the controller, and then rotate the cam latch all the way up to lock the module in place.
. Cable the I/O module.

. Repeat the remove and install steps to replace additional I/O modules for the controller.
