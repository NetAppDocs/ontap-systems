...
. On the controller A, remove the I/O blanking module from the target slot.
+
...
a|
Pull the I/O blanking module out of the controller using the handle on the left and the thumbscrew.
...
+
NOTE: Make sure that any unused I/O slots have blanking module installed to prevent possible thermal issues.

. Reboot the controller from the LOADER prompt: _bye_
+
NOTE: This reinitializes the I/O modules, other components, and reboots the node.
+
...
. On the controller A, unplug any cabling on the target I/O module.
...
. Repeat the remove and install steps to replace any additional I/O modules for the controller.
