To replace the controller, you must remove the impaired controller, move FRU components from the impaired controller to the replacement controller, install the replacement controller in the chassis, set the time and date, and the recable the replacement controller.
...
+
You might need to slightly rock the connector from end to end (lengthwise) until it is fully seated.
+
...
NOTE: Depending on your storage system configuration you will have two or four DIMMs.
a|
