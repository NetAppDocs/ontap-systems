...
+ 
- If the command returns `There are no entries matching your query.` continue to <<Check-the-amber-NVRAM,go to the next step>>. 
- If the command returns any other results, collect the AutoSupport data from both controllers and contact NetApp Support for further assistance.
+
[source,cli]
----
...
. [[Check-the-amber-NVRAM]]Check the amber NVRAM that status LED located in slot 4/5 on the back of the impaired controller module is off. Look for the NV icon.
+
