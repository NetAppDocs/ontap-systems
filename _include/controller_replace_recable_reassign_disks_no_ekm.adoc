...
. Retrieve and verify the clusterwide passphrase from the healthy controller.
.. Run the following command to retrieve the clusterwide passphrase:
+
`security key-manager onboard show-backup`
+
.. Verify the clusterwide passphrase using the following command:
+
