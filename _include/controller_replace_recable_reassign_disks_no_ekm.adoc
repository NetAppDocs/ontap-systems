...
Before starting, review https://docs.netapp.com/us-en/ontap-cli/security-key-manager-onboard-show-backup.html[security key-manager onboard show-backup] or contact NetApp Support.
...
.. Run the following command to display the key management backup information for the cluster:
+
`security key-manager onboard show-backup`
+
.. Run the following command to verify the onboard key manager backup:
+
`security key-manager onboard verify-backup`
. Enter the OKM passphrase and confirm successful verification.
+
...
+
.. Verify status of the keys: `security key-manager key query -restored false`
+
