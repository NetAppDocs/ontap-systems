...
//Created generic lead to topic for both no encryption and OKM procedures. 6/4/2024 - DougT
...
. Retrieve and verify the cluster-wide passphrase from the healthy controller.
.. Run the following command to retrieve the cluster-wide passphrase:
+
`security key-manager onboard show-backup`
+
.. Verify the cluster-wide passphrase using the following command:
+
...
--
// EKM not affected - With a controller replacement on these systems, the entire EKM configuration is untouched because nothing changes with SSN, system ID or contents of the boot device (/var). Only OKM requires restoration because of the new TPM chip on the new motherboard.
