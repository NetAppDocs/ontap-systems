You can add an I/O module to your storage system when there are empty slots available or when all slots are fully populated.
...
If the storage system has empty slots, add the I/O modules. If the storage system has no empty slots, replace an existing I/O module with a different one. 
