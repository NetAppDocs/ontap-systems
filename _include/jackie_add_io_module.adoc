You can add an I/O module to your storage system by adding a new I/O module into a storage system with empty slots or by migrating an I/O module to a new one in a fully-populated storage system.
...
== Step 2: Add or migrate the I/O module
If the storage system has empty slots, add the I/O modules. If the storage system has no empty slots, migrate an existing I/O module to a new one. 
...
.Add new I/O module (empty slots)
--
Add a new I/O module into a storage system with empty slots.
...
image::../media/drw_a1k_io_remove_replace_ieops-1382.svg[Remove a AFF A1K I/O module]
+
[cols="1,4"]
|===
a|
...
.Migrate I/O module (pre-populated slots)
--
You can migrate an I/O module in an I/O slot in a fully-populated system by removing an existing I/O module and replacing it with a different I/O module.

.About this task
Make sure you understand the following migration scenarios:

**Migrate from NIC to NIC**

- Migrating a NIC I/O module with the same number of ports
+
The LIFs will automatically migrate when its controller module is shut down.

- Migrating a NIC I/O module with fewer ports or more
+
...
**Migrate NIC to storage**

- Migrating a NIC I/O module to a storage I/O module
+
...
image::../media/drw_a1k_io_remove_replace_ieops-1382.svg[Remove AFF A1K I/O module]
+
[cols="1,4"]
|===
a|
...
+
* If you migrated a NIC I/O module to a new NIC I/O module, use the following network command for each port:
+
`storage port modify -node *_<node name>__ -port *_<port name>__ -mode network`

* If you migrated a NIC I/O module to a storage I/O module, install and cable your NS224 shelves, as described in link:../ns224/hot-add-shelf-overview.html[Hot-add workflow].
