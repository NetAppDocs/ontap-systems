...
== Option 1: Add the I/O module to a storage system with empty slots
...
. Unplug the power supply units (PSUs) cables from the PSUs from the controller.

+
NOTE: If your storage system has DC PSUs, disconnect the power cable block from the PSU.
+
...
. If you are not already grounded, properly ground yourself.
. Unplug the PSU cables from the PSUs from the controller.
