...
+
`event log show -node local -message-name \*hotplug.init*`
+
...
+
For example, you will see similar output shown below:
----
node1::*> system controller config show -node _node1_ -slot 4
