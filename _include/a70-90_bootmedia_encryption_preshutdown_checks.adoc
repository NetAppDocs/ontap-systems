 ...
 ** If <lno-DARE> or <1Ono-DARE> is displayed in the command output, the system does not support NVE, proceed to link:../a70-90/bootmedia-shutdown.html[Shut down the impaired controller].
 ** If <lno-DARE> is not not displayed in the command output, and the system is running ONTAP 9.6 or later, continue to the next section, <<Check NVE or NSE on systems running ONTAP 9.6 and later>>.  *COMMENT* We should create a version of this for 9.15 and later (part of the discussion
 )
...
include::../_include/encryption_check_9_15_later.adoc[]