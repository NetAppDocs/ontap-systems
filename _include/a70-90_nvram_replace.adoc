...
WARNING: The NVRAM module is not hot-swappable. Before replacing it, reseat the drives, shut down the node, unplug controller power, and pull the controller a few inches out of the chassis.
...
+ 
- If the command returns `There are no entries matching your query.` continue to <<Go-to-the-rear,go to the next step>>. 
- If the command returns any other results, collect the AutoSupport data from both controllers and contact NetApp Support for further assistance.
+
[source,cli]
----
...
. [[Go-to-the-rear]]Go to the rear of the chassis. 
...
. Reinstall the controller module. The controller reboots as soon as it is fully inserted. 
...
To replace NVRAM DIMMs in the NVRAM module, first you must remove the NVRAM module and then replace the target DIMM.
