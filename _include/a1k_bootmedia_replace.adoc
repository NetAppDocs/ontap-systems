To replace the boot media, you must remove the System Management module from the back of the system, remove the impaired boot media, install the replacement boot media in the System Management module, and transfer the ONTAP image from a USB flash drive to the replacement boot media.
...
The replacement boot media that you installed is without a ONTAP image so you need to transfer a ONTAP image using a USB flash drive.

.Before you begin

 * You must have a an empty USB flash drive, formatted to FAT32, with at least 4GB capacity.
...
+
  ...
  .. Copy the efi folder to the top directory (root) on the USB flash drive.
  +
 The USB flash drive should have the efi folder and the appropriate ONTAP image of what the impaired controller is running.
