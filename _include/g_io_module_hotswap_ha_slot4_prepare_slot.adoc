...
+
`system node autosupport invoke -node * -type all -message MAINT=<_number of hours down>h`
+
...
. Prepare I/O module slot 4 on the impaired controller (the controller that took over the healthy controller) for removal:
+
