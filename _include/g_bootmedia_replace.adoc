...
.. Slide the socket (left) end of the boot media into its socket.
.. At the opposite (right) end of the boot media, press down and hold the blue tab (in the open position), gently push down on that end of the boot media so it touches the motherboard, and then release the tab to lock the boot media into place.
...
== Step 5: Transfer the boot image to the boot media
