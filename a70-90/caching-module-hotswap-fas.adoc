---
permalink: a70-90/caching-module-hot-swap-fas.html
sidebar: sidebar
keywords: fas70-90, FAS70-90, FAS70, FAS90, hot swap, replace, FlashCache
summary: "The NVMe SSD FlashCache modules (FlashCache or caching modules) is located  in Slot 6 of FAS70-90 systems only. You can hot-swap the caching module of the same capacity from the same or different supported vendor."
---
= Hot-swap a caching module - FAS70-90
:icons: font
:imagesdir: ../media/

[.lead]
The following instructions describe service the caching (flashcache) module on your system.

The caching module consists of two FRU components:
* The drive module with the NVMe drive.
*The caching module carrier that connects the NVMe carrier to the system.  

The caching module is located  in Slot 6 of FAS70-90 systems only. You can replace the drive module of the caching module of the same capacity from the same or different supported vendor.
...
* You must shut down and remove power from the impaired controller. The caching components are not hot-swappable.
...
. Locate the failed caching module, in slot 6, by the lit amber Attention LED on the bezel of the caching module.
