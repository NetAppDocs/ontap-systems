...
.About this task
If you have a storage system configured with two 2-port 100GbE I/O modules (in slots 2 and 4) for the cluster/HA connections, this step does not apply??? Or are they using the left over "b" ports??? Are hots network connections "front-end" connections, that's what Andris said they're used for.
