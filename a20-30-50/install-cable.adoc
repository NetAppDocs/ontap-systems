...
The ports on I/O modules in slot 2 and 4 are 40/100GbE ports. 

Does this apply only to 30-50, if yes ( I think it's YES) leave the 2 notes where the are, but if applies to all 3, then move it to new paragraph below section lead text.???
...
Applies just to switchless or here too???The I/O module ports in slot 2 and 4 are 40/100GbE ports.???? Does this apply only to 30-50, if yes leave the 2 notes where the are, but if applies to all 3, then move it to new paragraph below section lead text.???
...
+
NOTE: I/O module ports e2b and e4b are unused and available for hot network (front-end) connectivity, if needed.
+
...
.. Optional: Cable controllers to 40/100GbE host network switches.
+
