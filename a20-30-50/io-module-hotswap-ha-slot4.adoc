...
* Hot-swapping the cluster and HA I/O module means that you do not have to perform a manual takeover; the impaired controller has automatically taken over the healthy controller.
+
When the controller with the failed cluster and HA I/O module (the impaired controller) has taken over the healthy controller, the only way to recover without an outage is to hot-swap the module.
