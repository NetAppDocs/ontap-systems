...
The hot-swap process involves preparing the storage system and I/O module slot 4 so that it's safe to remove the failed module, hot-swapping the failed module for an equivalent one, bringing the replacement module online, and returning the failed module to NetApp.
...
** The controller with the failed cluster and HA I/O module (the impaired controller) must have taken over the healthy HA partner controller. The takeover should have occured automatically if the I/O module is failed.
+
