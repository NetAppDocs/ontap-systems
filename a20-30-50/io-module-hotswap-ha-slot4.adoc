...
---
= Hot-swap the I/O module used for cluster and HA traffic - AFF A20, AFF A30, and AFF A50
:icons: font
:imagesdir: ../media/
...
[.lead]
The cluster and HA I/O module supports interconnects for clustering and high-availability. You can hot-swap the module in your AFF A20, AFF A30, or AFF A50 storage system when the module fails and when your storage system meets specific requirements.
...
** The controller with the failed cluster and HA I/O module (the impaired controller) must have taken over the healthy partner controller. The takeover should have occured automatically if the I/O module is failed.
+
For two-node clusters, the storage system cannot discern which controller has the the failed I/O module, so either controller might initiate the takeover. The cluster and HA I/O module hot-swap procedure is only supported when the controller with the failed I/O module (the impaired controller) has taken over the healthy controller.
+
...
* Hot-swapping the cluster and HA I/O module means that you do not have to perform any controller takeover steps; the impaired controller has already taken over the healthy controller.
